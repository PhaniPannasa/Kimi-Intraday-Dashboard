...
describe('useWebSocket', () => {
  ...
  it('should handle L8_THESIS messages', () => {
    ...
    mockWs.onmessage?.({ data: JSON.stringify({
      ...
      payload: { thesis_id: 't1', card: { thesis_id: 't1', symbol: 'RELIANCE', direction: 'LONG', setup_type: 1, trigger: 2500, invalidation: 2450, t1: 2550, t2: 2600, gross_rr: 2.0, net_rr: 1.8, grade: 'ATTRACTIVE', confluence_score: 5, time_decay_multiplier: 1.0, actionability_tier: 'Tradeable', valid_until: '2026-05-17T10:00:00Z', preferred_regime: 'Trending-Up' } },
    })});
  ...
  it('should handle L9_INVALIDATION messages', () => {
    renderHook(() => useWebSocket());
    mockWs.onopen?.();
    useMarketStore.getState().addOrUpdateThesis({ thesis_id: 't2', symbol: 'INFY', direction: 'SHORT', setup_type: 1, trigger: 1500, invalidation: 1550, t1: 1450, t2: 1400, gross_rr: 1.5, net_rr: 1.3, grade: 'MARGINAL', confluence_score: 3, time_decay_multiplier: 0.9, actionability_tier: 'Constrained', valid_until: '2026-05-17T10:00:00Z', preferred_regime: 'Trending-Down' });
